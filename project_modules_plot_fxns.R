...
plot_module_projections <- function(projectname, df, cellinfo, expr, expr_type, pval_cut, 
  ...
  if(pval_cut < max(df$Pval)) df <- df[df$Pval < pval_cut,]
  ...
  pdf(file=file_path, height=19, width=14)
  ...
  lapply(unique(df$Network), FUN=function(network){
    ...
    lapply(unique(df1$Module), function(mod){
      ...
      p_seed <- tableGrob(d=seed_df, theme=ttheme_minimal(base_size=13, padding=unit(c(2.5, 2.5), "mm")), rows=NULL)
...
plot_rel_group_module_projections <- function(projectname, df, cellinfo, expr, expr_type, pval_cut, 
                                              n_genes, setsource=c("all", "MO", "BROAD")){
  ...
  file_path <- paste0("figures/", projectname, "_", expr_type, "_rel_group_module_projections_", setsource, "_sets_pval_cut_", pval_cut, "_top_", n_genes, "_module_genes.pdf")
  
  pdf(file=file_path, height=17, width=14)
  ...
  lapply(unique(df$Network), FUN=function(network){
    ...
    lapply(unique(df1$Module), function(mod){
      ...
      y_lims <- c(-.1, max(df2$Err_Max) + .1)
      ## Facet by cell type:
      p1 <- ggplot(df2, aes(x=Group, y=Projection_Index, fill=EGFR_Status)) +
        geom_bar(stat="identity", color="black") +
        geom_errorbar(aes(ymin=Err_Min, ymax=Err_Max), width=.2) + 
        theme_minimal() +
        theme(plot.subtitle=element_text(margin=margin(b=7)),
