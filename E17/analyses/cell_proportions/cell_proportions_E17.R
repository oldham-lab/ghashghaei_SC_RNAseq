setwd("/mnt/bdata/rebecca/collabs/ghashghaei/SC_expr/E17/analyses/cell_proportions")
...
df <- expr[[]] %>%
  ...
  dplyr::group_by(customclassif, Group, Background, Condition) %>%
  dplyr::summarise(n=n()) %>%
  dplyr::group_by(Group) %>%
  dplyr::mutate(Frac=n/sum(n))

df$Group <- factor(df$Group, levels=c("WT Td", "WT GFP", "FF Td", "FF GFP", "F+ Td", "F+ GFP"))
df$Condition <- factor(df$Condition, levels=c("WT", "Null"))
# df$Background <- factor(df$Background, levels=c("Homo", "Het"))
...
pdf("figures/E17_cells_by_group_and_cell_type_barplot.pdf", width=12, height=7)

print(
  ggplot(df, aes(x=Group, y=Frac, fill=Condition)) +
