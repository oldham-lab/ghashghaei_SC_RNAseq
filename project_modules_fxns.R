...
project_modules_rel_group <- function(projectname, expr, fm_dir, expr_type, pval_cut, n_genes){
  ...
  proj_list <- lapply(unique(signif_mods$Network), FUN=function(network){
    ...
    names(mod_genes) <- unique(signif_mods1$Module)
    ## For each cell type:
    clust_proj_list <- lapply(1:length(clust_index), function(j){
      ## Stratify expression data by group:
      group_index1 <- lapply(group_index, function(x) intersect(x, clust_index[[j]]))
      mat_group <- future_lapply(group_index1, FUN=function(index) as.matrix(mat[,index]))
      all_mean <- unlist(lapply(mat_group, mean))
      ## For each module:
      mod_proj_list <- future_lapply(1:length(mod_genes), FUN=function(k){
        ## Project modules onto cells by taking mean expression of module genes over all cells per group:
        mod_mean <- unlist(lapply(mat_group, function(mat){
          return(mean(mat[rownames(mat) %in% mod_genes[[k]],]))
        }))
        std_err <- unlist(lapply(mat_group, function(mat){
          ## Std err of cell mean expression over module genes:
          x <- colMeans(mat[rownames(mat) %in% mod_genes[[k]],])
          return(sd(x)/sqrt(length(x)))
        }))
        ...
        proj_index <- mod_mean/all_mean
        std_err <- std_err/all_mean
        ## Normalize proj. index relative to all experimental groups:
        proj_index <- proj_index/max(proj_index)
        std_err <- std_err/max(proj_index)
        return(data.frame(Module=names(mod_genes)[k], 
                          Group=names(proj_index),
                          No.Nuclei=unlist(lapply(mat_group, ncol)), 
                          Projection_Index=proj_index, 
                          Std_Err=std_err))
      })
      temp <- do.call(rbind, mod_proj_list)
      signif_mods1 <- signif_mods1[,c("Module", "Pval", "SetID", "SetName", "Network", "kME")]
      mod_proj <- merge(signif_mods1, temp, by="Module")
      return(data.frame(Cell_Type=names(clust_index)[j], mod_proj))
    })
    return(do.call(rbind, clust_proj_list))
  })
  df <- do.call(rbind, proj_list)
  fwrite(df, file=paste0("data/", projectname, "_", expr_type, "_rel_group_module_projections_pval_cut_", pval_cut, "_top_", n_genes, "_module_genes.csv"))
